								...
								clearButton3(juce::CharPointer_UTF8("3\xf0\x9f\x9e\xab"))
{
	...
	clearButton1.setEnabled(false);  // alapértelmezett állapotban letiltva
	...
	clearButton2.setEnabled(false);  // alapértelmezett állapotban letiltva
	...
	clearButton3.setEnabled(false);  // alapértelmezett állapotban letiltva
...
void MainComponent::clearButtonClicked()
{
	...
	maxBufferSize = 44100 * 60; // max 60 sec mono buffer per channel
...
void MainComponent::playButton1Clicked()
{
	...
	if (empty1 == true && recording1 == false && playing1 == false)  // felvétel
	{
		...
		{
			globalSamplePosition = 0;
			readPositions[0] = 0; // Reset read position
		}
		...
		clearButton1.setEnabled(true);  // alapértelmezett állapotban letiltva
	...
	else if (empty1 == false && recording1 == true && playing1 == false && empty2 && empty3)  // köztes szakasz
	{
		...
		writePositions[0] = 0; // Reset write position
...
void MainComponent::playButton2Clicked()
{
	...
	if (empty2 == true && recording2 == false && playing2 == false)  // felvétel
	{
		...
		{
			globalSamplePosition = 0;
			readPositions[1] = 0; // Reset read position
		}

		stopButton2.setEnabled(true);
		allButton.setEnabled(true);  // alapértelmezett állapotban letiltva
		clearButton.setEnabled(true);  // alapértelmezett állapotban letiltva
		clearButton2.setEnabled(true);  // alapértelmezett állapotban letiltva
...
void MainComponent::playButton3Clicked()
{
	...
	if (empty3 == true && recording3 == false && playing3 == false)  // felvétel
	{
		...
		{
			globalSamplePosition = 0;
			readPositions[2] = 0; // Reset read position
		}

		stopButton3.setEnabled(true);
		allButton.setEnabled(true);  // alapértelmezett állapotban letiltva
		clearButton.setEnabled(true);  // alapértelmezett állapotban letiltva
		clearButton3.setEnabled(true);  // alapértelmezett állapotban letiltva
	...
	else if (empty3 == false && recording3 == true && playing3 == false && empty1 && empty2)  // köztes szakasz
	{
		...
		writePositions[2] = 0; // Reset write position
...
void MainComponent::prepareToPlay(int samplesPerBlockExpected, double sampleRate)
{
	...
	for (int i = 0; i < 3; ++i)
	{
		channelBuffers[i].setSize(1, maxBufferSize); // mono
		channelBuffers[i].clear();
		writePositions[i] = 0;
		readPositions[i] = 0;
		hasData[i] = false;
	...
	for (int sample = 0; sample < bufferToFill.numSamples; ++sample)
	{
		...
		for (int ch = 0; ch < 3; ++ch)
		{
			...
			if (isRec && writePositions[ch] < maxBufferSize)
			{
				...
				if (loopLengthLocked)
				{
					// Latency-kompenzált overdub pozíció
				...
				{
					// Első felvétel — sima lineáris írás
		...
		// Globális minta számláló frissítése
